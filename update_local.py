...
def update_local(competition, season):
    ...
    bundesliga_ignore = set(
        [
            "5dc40876", "9c6a24db", "e262266b", "f5e7a5c2", "434865ef", "2f2a35fa",
            "dc47142c", "948872ab", "ac3eb7f6", "d50b48fe"
        ]
    )

    ligue1_ignore = set(
        [
            "f927719d", "06517ea5", "bfd434c1", "ea5db1c4", "621f8a81", "28ce9808",
            "f1560d55", "78ed8c0d"
        ]
    )
    ...
    stored_files = set(os.listdir(
        os.path.join("web_pages", competition, season)))
    ...
    with requests.Session() as s:
        for number, match_id in enumerate(missing_match_ids):
            ...
            with open(os.path.join("web_pages", competition, season, match_id), 'w', encoding="utf-8") as f:
                f.write(resp)
